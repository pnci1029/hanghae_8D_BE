...
//ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ댓글ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ//
    ...
    public Comment createComment(long articlesId, String comment, Type type) {
        Optional<Article> target = articleRepository.findById(articlesId);
        Optional<Article> target2 = articleRepository.findById(12L);
...
//        유저 2가작성한 댓글
        for (int i = 0; i < 11; i++) {
            comments = Comment.builder()
                    ...
                    .type(type)
                    .article(target.get())
                    .member(target2.get().getMember())
                    .nickName(target2.get().getMember().getNickName())
                    .userRank("S")
                    .isMyComment(false)
                    .isSelected(false)
                    .build();
...
//        동일 사용자 게시글 댓글 작성 예외처리
        ...
        Comment comment = targetComment.orElse(null);
        comment.setSelected(true);
        comment.setUserRank("AAA");
    ...
    public String deleteComment(Long commentId) {
        ...
        if (targetComment.isEmpty()) {
            throw new NullPointerException("존재하지 않은 댓글입니다");
        }
        commentRepository.delete(targetComment.orElse(null));
        return "삭제가 완료되었습니다.";
    }
}
